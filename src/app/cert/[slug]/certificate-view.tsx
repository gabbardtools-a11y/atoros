        ...
        <div className="relative h-full flex flex-col" style={{ zIndex: 10 }}>
          ...
          <div className="grid gap-3 flex-1" style={{ gridTemplateColumns: '1fr 56mm' }}>
            ...
            >
              ...
              {/* FNS trusted timestamp block (RFC 3161, GOST R 34.11-2012) */}
              {cert.fnsTimestampTime && cert.fnsGostHash && (
                <div className="mt-3 pt-3 border-t" style={{ borderColor: 'rgba(176,138,62,0.2)' }}>
                  <div className="flex justify-between items-baseline mb-2">
                    <div className="text-[11px] font-bold" style={{ color: '#0B1220' }}>
                      ФНС <span className="text-[8.5px] font-medium" style={{ color: '#B08A3E' }}>· метка доверенного времени · RFC 3161</span>
                    </div>
                    <div className="font-mono text-[8px] text-slate-400">/ 03</div>
                  </div>
                  <div
                    className="px-2 py-1.5 rounded"
                    style={{
                      background: 'rgba(176,138,62,0.06)',
                      border: '0.5px solid rgba(176,138,62,0.25)',
                    }}
                  >
                    <div className="text-[8.5px] font-bold mb-1" style={{ color: '#7A5B1E' }}>
                      Квалифицированная метка ФНС · ГОСТ Р 34.11-2012
                    </div>
                    <div className="grid grid-cols-2 gap-1.5">
                      <div>
                        <div className="text-[7px] uppercase tracking-wider text-slate-500">Время фиксации</div>
                        <div className="font-mono text-[8.5px] font-medium" style={{ color: '#7A5B1E' }}>
                          {new Date(cert.fnsTimestampTime).toLocaleString('ru-RU', {
                      ...
                      <div>
                        <div className="text-[7px] uppercase tracking-wider text-slate-500">Серийный номер</div>
                        <div className="font-mono text-[7.5px] font-medium break-all" style={{ color: '#7A5B1E' }}>
                          {cert.fnsTokenSerial}
                        </div>
                      </div>
                    </div>
                    <div className="mt-1.5">
                      <div className="text-[7px] uppercase tracking-wider text-slate-500">GOST R 34.11-2012 (Streebog) 256-bit</div>
                      <div className="font-mono text-[7.5px] leading-relaxed break-all" style={{ color: '#1D4ED8' }}>
                        {cert.fnsGostHash}
                      </div>
                    </div>
                  </div>
                  <div className="font-mono text-[7px] text-slate-400 mt-1.5">
                    <span className="uppercase tracking-wider text-slate-500 mr-1">TSP-сервер:</span>
                    uc.nalog.ru/tsp/tsp.srf · ФНС России
                  </div>
                </div>
              )}

              {/* Torrent (BitTorrent) block — decentralized storage */}
              {cert.torrentBtih && cert.torrentMagnet && (
                <div className="mt-3 pt-3 border-t" style={{ borderColor: 'rgba(16,185,129,0.2)' }}>
                  <div className="flex justify-between items-baseline mb-2">
                    <div className="text-[11px] font-bold" style={{ color: '#0B1220' }}>
                      Torrent <span className="text-[8.5px] font-medium" style={{ color: '#10b981' }}>· децентрализованное хранение · BitTorrent</span>
                    </div>
                    <div className="font-mono text-[8px] text-slate-400">/ 04</div>
                  </div>
                  <div
                    className="px-2 py-1.5 rounded"
                    style={{
                  ...
                  >
                    <div className="text-[8.5px] font-bold mb-1" style={{ color: '#047857' }}>
                      Архив в торрент-сети · всегда доступен
                    </div>
                    <div className="grid grid-cols-2 gap-1.5">
                      <div>
                        <div className="text-[7px] uppercase tracking-wider text-slate-500">Info Hash (btih)</div>
                        <div className="font-mono text-[7.5px] font-medium break-all" style={{ color: '#047857' }}>
                          {cert.torrentBtih}
                        </div>
                      </div>
                      <div>
                        <div className="text-[7px] uppercase tracking-wider text-slate-500">Создан</div>
                        <div className="font-mono text-[8px]" style={{ color: '#047857' }}>
                          {cert.torrentCreatedAt ? new Date(cert.torrentCreatedAt).toLocaleString('ru-RU', {
                            day: '2-digit', month: '2-digit', year: 'numeric',
                    ...
                    <div className="mt-2 flex flex-wrap gap-1.5">
                      <a
                        href={cert.torrentMagnet}
                        className="inline-block text-[7.5px] font-semibold px-2 py-1 rounded text-white"
                        style={{ background: '#10b981' }}
                      >
                        Magnet-ссылка
                      </a>
                      <a
                        href={`/api/torrent?slug=${cert.slug}`}
                        className="inline-block text-[7.5px] font-semibold px-2 py-1 rounded"
                        style={{ color: '#047857', border: '0.5px solid #10b981', background: 'white' }}
                      >
                        Скачать .torrent
                      </a>
                    </div>
                  ...
                  <div className="font-mono text-[7px] text-slate-400 mt-1.5">
                    <span className="uppercase tracking-wider text-slate-500 mr-1">Клиент:</span>
                    transmission-daemon 4.1.1 · 6 публичных трекеров · DHT + PEX
                  </div>
                </div>
              )}
            </div>
          ...
          <div className="mt-4 pt-3 border-t" style={{ borderColor: 'rgba(15,23,42,0.1)' }}>
            ...
            {/* Cloud storage links */}
            {(cert.yandexDiskUrl || cert.googleDriveUrl) && (
              <div className="mt-3 pt-2 border-t" style={{ borderColor: 'rgba(15,23,42,0.1)' }}>
                <div className="text-[8px] font-bold tracking-[2.4px] uppercase mb-2" style={{ color: '#1D4ED8' }}>
                  Облачные копии архива
                </div>
                <div className="grid grid-cols-1 gap-2">
                  {cert.yandexDiskUrl && (
                    <a
                      href={cert.yandexDiskUrl}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex items-center gap-2 p-2.5 bg-white rounded transition hover:bg-blue-50"
                      style={{ border: '0.5px solid rgba(15,23,42,0.1)', boxShadow: '0 1px 2px rgba(15,23,42,0.03)' }}
                    >
                      <span className="text-[9px] font-bold" style={{ color: '#FC3F1D' }}>Я</span>
                      <div className="flex-1 min-w-0">
                        <div className="text-[9px] font-medium text-slate-700">Яндекс.Диск</div>
                        <div className="font-mono text-[7px] text-slate-400 truncate">{cert.yandexDiskUrl}</div>
                      </div>
                  ...
                  {cert.googleDriveUrl && (
                    <a
                      href={cert.googleDriveUrl}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex items-center gap-2 p-2.5 bg-white rounded transition hover:bg-blue-50"
                      style={{ border: '0.5px solid rgba(15,23,42,0.1)', boxShadow: '0 1px 2px rgba(15,23,42,0.03)' }}
                    >
                      <span className="text-[9px] font-bold" style={{ color: '#1A73E8' }}>G</span>
                      <div className="flex-1 min-w-0">
                        <div className="text-[9px] font-medium text-slate-700">Google Drive</div>
                        <div className="font-mono text-[7px] text-slate-400 truncate">{cert.googleDriveUrl}</div>
                      </div>
                      <ExternalLink className="h-3 w-3 text-slate-400 flex-shrink-0" />
                    </a>
                  )}
